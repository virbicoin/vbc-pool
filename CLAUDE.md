...
VBC Pool — VirBiCoin (VBC) ネットワーク向けのオープンソースマイニングプール。Go バックエンドと Next.js フロントエンドで構成されています。
...
| **vbc-pool** ← 本リポジトリ | マイニングプール | `../open-virbicoin-pool` | [github.com/virbicoin/vbc-pool](https://github.com/virbicoin/vbc-pool) |
| **vbc-rpc** | RPC ノードステータス & JSON-RPC プロキシ | `../vbc-rpc` | [github.com/virbicoin/vbc-rpc](https://github.com/virbicoin/vbc-rpc) |
| **ethash** | Ethash アルゴリズム実装（C/Go バインディング） | `../ethash` | [github.com/virbicoin/ethash](https://github.com/virbicoin/ethash) |

### 依存関係

- **openvirbicoin**: go-virbicoin（Gvbc）と同じ VirBiCoin ネットワーク（chainId 329）に接続する代替クライアント（Ovbc, Rust 実装）
- **vbc-pool** → **go-virbicoin**: マイニングプールが Gvbc ノードから作業を取得・ブロック解除
- **go-virbicoin** → **ethash**: Ethash PoW アルゴリズムの実装を利用
